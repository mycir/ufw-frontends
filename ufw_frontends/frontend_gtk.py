...
import ufw_frontends.event as event
import ufw_frontends.util as util
from ufw_frontends.frontend import Frontend
...
class Notifier(event.Notifier):
    def __init__(self, callback, inactive_handler):
        self._active = False
        try:
            event.Notifier.__init__(self, callback)
        except IOError:
            inactive_handler()
            return
        self._active = True
        self._w = GObject.io_add_watch(
            self._fd, GObject.IO_IN | GObject.IO_PRI, self._trigger
        )

    def __del__(self):
        if self._active:
            event.Notifier.__del__(self)
...
class GtkFrontend(Frontend):
    ...
    def __init__(self):
        ...
        self.ui = Builder()
        path = util.get_ui_path(self.UI_FILE)
        ...
        def callback(data, notify=True):
            ...
            #     _("Firewall"),
            #     _("Blocked incoming connection from %s") % (conn["SRC"],),
            #     gtk.STOCK_INFO,
            # )
            # n.show()
        ...
        self._notifier = Notifier(
            callback, lambda: self.ui.events_view.set_sensitive(False)
        )
    ...
    def _init_prefs_dialog(self):
        ...
        # Get current values
        logging_opts = list(
            map(str.lower, self._get_combobox_values("logging_cbox"))
        )
        policy_opts = list(
            map(str.lower, self._get_combobox_values("incoming_policy_cbox"))
        )
        logging = logging_opts.index(conf["loglevel"])
        default_incoming = policy_opts.index(
            self.backend.get_default_policy("input")
        )
        default_outgoing = policy_opts.index(
            self.backend.get_default_policy("output")
        )
    ...
    def _set_combobox_value(self, name, value):
        values = list(map(str.lower, self._get_combobox_values(name)))
        try:
    ...
    def _set_statusbar_text(self, text):
        ...
        GObject.timeout_add_seconds(5, self.ui.statusbar.remove, cid, mid)
    ...
    def _update_conns_model(self):
        ...
        util.get_connections(self.ui.conns_model.append)
        return True
    ...
    def on_prefs_dialog_show_activate(self, action):
        ...
        if self.ui.prefs_dialog.run() == self.RESPONSE_OK:
            ...
            # FTP
            self.config_ipt_module(
                "nf_conntrack_ftp", self.ui.mod_ftp_chkbox.get_active()
            )
            self.config_ipt_module(
                "nf_nat_ftp", self.ui.mod_ftp_chkbox.get_active()
            )
            # IRC
            self.config_ipt_module(
                "nf_conntrack_irc", self.ui.mod_irc_chkbox.get_active()
            )
            self.config_ipt_module(
                "nf_nat_irc", self.ui.mod_irc_chkbox.get_active()
            )
            # NetBIOS
            self.config_ipt_module(
            ...
            # PPTP
            self.config_ipt_module(
                "nf_conntrack_pptp", self.ui.mod_pptp_chkbox.get_active()
            )
            # saned
            self.config_ipt_module(
                "nf_conntrack_sane", self.ui.mod_saned_chkbox.get_active()
            )
            # reload firewall
            self.reload()
    ...
    def on_firewall_reset_activate(self, action):
        ...
        res = self._show_dialog(
            msg, type=Gtk.MessageType.WARNING, buttons=Gtk.ButtonsType.YES_NO
        )
    ...
    def on_firewall_update_activate(self, action):
        ...
        self._show_dialog(res, type=Gtk.MessageType.INFO)
    ...
    def on_rule_delete_activate(self, action):
        ...
        res = self._show_dialog(
            msg, type=Gtk.MessageType.QUESTION, buttons=Gtk.ButtonsType.YES_NO
        )
    ...
    def on_rule_up_activate(self, action):
        ...
        self.ui.rules_view.scroll_to_cell(new - 1, None, False, 0.0, 0.0)
    ...
    def on_event_allow_activate(self, action):
        ...
        self._restore_rule_dialog_defaults()
        # position
        self.ui.position_adjustment.set_value(
            self.ui.position_adjustment.get_upper()
        )
        # protocol
    ...
    def on_view_switch_page(self, widget, page, page_num):
        if page_num == 2:
            ...
            self._conn_timer = GObject.timeout_add_seconds(
                5, self._update_conns_model
            )
    ...
    def _app_info_clicked(self, prefix):
        ...
        if app is not None:
            ...
            self._show_dialog(info, self.ui.rule_dialog, Gtk.MessageType.INFO)
